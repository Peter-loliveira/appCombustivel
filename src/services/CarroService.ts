...
export class CarroService implements ICarroService {
  ...
  cadastrar(carro: Carro): Observable<Carro> {
    ...
    if (!carro.consumoAlcool)
      throw new Error("O capo de COMSUMO NO ALCOOL é obrigatório!");
    if (!carro.consumoGasolina)
      throw new Error("O capo de COMSUMO NO GASOLINA é obrigatório!");
  ...
  calcularLitros(
    ...
    if (!distancia) throw new Error("Digite a distancia a ser percorrida.");
